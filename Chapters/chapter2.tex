...
\begin{proof}
  ...
  \[ \P\{\|P_n-P\| > \varepsilon\} \leq 2 \P\{\|P_n-P_n'\| > \tfrac{1}{2} \varepsilon\},\hspace*{1em} \text{if $n \geq 8\varepsilon^{-2}$}. \tag*{(1)} \] 
  ...
  \begin{lemma}
    Let $\sigma_1, \ldots, \sigma_n$ be Rademacher random variables, that is $\P\{\sigma_i = 1\} = \P\{\sigma_i = -1\} = 1/2$. Let $Y_i = \1_{\{ X_i' < t\}} - \1_{\{ X_i < t\}}$, and note that,
    \[ \P\{Y_i = x\} = \P\{\sigma_i Y_i = x\},\hspace*{1em} x \in \{-1,0,1\} \] 
  \end{lemma}

  \begin{proof} In the first place, from the independency of $X_i$ from $X_i'$ and their equality of distributions, the following symmetry holds: 

    \[ \P\{Y_i = 1\} = \P\{Y_i = -1\} = \frac{1}{2} \left(1 - \P\{Y_i = 0 \}\right) = \frac{1}{2} \P\{X_i \neq X_i'\}. \]

  Since $\sigma_i$ is independent of $Y_i$, it follows that
  \[ \everymath{\displaystyle}\arraycolsep=1.8pt\def\arraystretch{1.8}
  \begin{array}{rcl}
  ...
  \end{array} \] 
  \[ = 2\P\{\|P_n^\circ\| > \tfrac{1}{4} \varepsilon\}. \tag*{(2)} \] 
  where $P_n^\circ = n^{-1}\sum_{i\leq n} \sigma_i \1_{\{ X_i < t\}}$. Then, from equations $(1), (2)$ we conclude that for $n \geq 8\varepsilon^{-2}$,
  ...
  For any given sample $X = X_1, \ldots, X_n$, define $X_{(j)}$ as the $j$-th observation when we order the observations, and choose $t_j \in (X_{(j)}, X_{(j+1)}]$ as the picture above shows. Note that if $t \in (X_{(j)}, X_{(j+1)}]$, then $P_n^\circ (t) = P_n^\circ (t_j)$ since, % chktex 9

  \[ \everymath{\displaystyle}\arraycolsep=1.8pt\def\arraystretch{1.8}
  \begin{array}{rcl}
    P_n^\circ (t) & = & n^{-1}\sum_{i = 1}^n \sigma_i\1_{\{ X_i < t\}}\\
    & = & n^{-1}\sum_{i = 1}^n \sigma_i\1_{\{ X_{(i)} < t\}}\\
    & = & n^{-1}\sum_{i = 1}^{j} \sigma_i\1_{\{ X_{(i)} < t\}} + \sum_{i = j+1}^{n} n^{-1} \sigma_i \1_{\{ X_{(i)} < t\}}\\
    & = & 0 + \sum_{i = j+1}^{n} n^{-1} \sigma_i = P_n^\circ (t_j).
  \end{array}
  \]

  It follows that for some $j$, $\|P_n^\circ\| = |P_n^\circ (j)|$, and thus,
  \[ \everymath{\displaystyle}\arraycolsep=1.8pt\def\arraystretch{1.8}
    \begin{array}{rcl}
      \P\{ \|P_n^\circ\| > \tfrac{1}{4} \varepsilon \;|\; X \} & \leq & \sum_{j = 1}^n\P\{ |P_n^\circ(j)| > \tfrac{1}{4} \varepsilon \;|\; X \}\\
      & \leq & n \cdot \max_j \P\{ |P_n^\circ(j)| > \tfrac{1}{4} \varepsilon \;|\; X \} \tag{3}.
    \end{array}\] 
  ...
  We use equation $(3)$ to conclude
  \[ \P\{ \|P_n^\circ\| > \tfrac{1}{4} \varepsilon \;|\; X\} \leq 2n e^{-n\varepsilon^2/32}.\] 

  \vspace*{1em}

  \subsubsection*{Integration}

  something is missing before the following step,

  \[ \P \{\|P_n-P\| > \varepsilon\} \leq  8n e^{-n\varepsilon^2/32}\] 

  Since the inequality is exponential, the probabilities are summable:
