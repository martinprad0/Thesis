...
\begin{proof}
  ...
  The following trick will allow us to stop considering all of the $2n$ from the previous symmetrization, and will help us to create a simpler random variable. Let $\sigma_1, \ldots, \sigma_n$ be Rademacher random variables, that is $\P\{\sigma_i = 1\} = \P\{\sigma_i = -1\} = 1/2$. Let $Y_i = \1_{\{ X_i < t\}} - \1_{\{ X_i < t\}}$, and note that,
  \[ P\{Y_i = 1\} = P\{Y_i = -1\} = \frac{1}{2} \left(1 - \P\{Y_i = 0 \}\right). \]
  ...
  \end{array}
    \] 
  \[ \implies P\{\sigma_i Y_i = \pm 1\} = P\{Y_i = \pm 1\},\hspace*{1em}  P\{\sigma_i Y_i = 0\} =P\{ Y_i = 0\} \] 
  Therefore, we won't alter the distribution of $Y_i$ when multiplying by $\sigma_i$
  \[ \implies Y_i \sim \sigma_i Y_i.\]
  ...
  \begin{array}{rcl}
    ...
    & = & 2\P\{\|\tilde{P_n}\| > \tfrac{1}{4} \varepsilon\}.
  \end{array} \] 
  where $\tilde{P_n} = n^{-1}\sum_{i\leq n} \sigma_i \1_{\{ X_i < t\}}$. Then, from the previous section we can conclude that for $n \geq 8\varepsilon^{-2}$,
  \[ \P\{\|P_n-P\| > \varepsilon\} \leq 4 \P\{\|\tilde{P_n}\|> \tfrac{1}{4}\varepsilon\}\] 
\end{proof}
