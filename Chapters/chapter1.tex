...
\begin{theorem}\label{centrallimit}
    Let $X_i$ be a i.i.d.~sample, each with expected value $\mu$ and variance $\sigma^2$. Let $S_N = \sum_{i = 1}^N X_i$ and $Z_N = \frac{S_N - N \mu}{\sqrt{N}\sigma^2}$. Then,
...
With that in mind, we might naively assume that we can obtain better bounds by using the previous theorems since,
